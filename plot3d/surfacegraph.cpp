...
void SurfaceGraph::setData(const MatrixXd & X, const MatrixXd& Y, const MatrixXd &Z){
    //创建数据对象
    QSurfaceDataArray *data = new QSurfaceDataArray;
    //为数据对象赋值
    for(int i=0; i<Z.size(); i+=10){
        QSurfaceDataRow *dataRow = new QSurfaceDataRow;
        for (int j = 0; j < Z.row(0).size(); j+=10)
            *dataRow<<QVector3D(X(i,j),Z(i,j),Y(i,j));
         *data << dataRow;
    }
    m_sqrtSinProxy->resetArray(data);
}
